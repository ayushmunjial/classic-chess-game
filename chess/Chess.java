...
public class Chess {
	...
	enum Player { white, black } public static Player turn; public static int movePlay; 
	...
	public static ReturnPlay play(String move) { 
		...
		ReturnPlay playGame = new ReturnPlay(); playGame.piecesOnBoard = Board.currPiecesOnBoard; playGame.message = null;
		return playGame;
	}
	...
	public static void parseMove(String move) { 
		...
		if(whiteSpaceCount==1 || whiteSpaceCount==2) {
			...
			for (ReturnPiece piece : Board.currPiecesOnBoard) { String f = oldPosition.substring(0, 1);
				if (piece.pieceFile.toString().equals(f) && piece.pieceRank == X2) { currReturnPiece = piece; }
			} 
	...
	public static void exec_Move(int X1, int Y1, int X2, int Y2, String command, ReturnPiece currReturnPiece) { 
		...
		Piece typeOfPiece = PieceOfType.createPiece(currReturnPiece); boolean isEmpty = false;

		// To check if the new position is empty.
		if (command.equals("")) { typeOfPiece.isValidMove(X1, Y1, X2, Y2, isEmpty); }
	}
}
